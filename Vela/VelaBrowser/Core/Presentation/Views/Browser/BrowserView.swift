...
struct BrowserView: View {
    ...
    @StateObject private var downloadAnimationManager = DownloadAnimationManager()
    ...
    // Track download folder position
    @State private var downloadFolderPosition: CGPoint = .zero
    ...
    var body: some View {
        ...
        ConfigurableSplitView(
            ...
            // Sidebar with download folder position tracking
            SidebarView(
                viewModel: viewModel,
                previewManager: previewManager,
                boardVM: viewModel.noteboardVM,
                ...
                if !viewModel.isInBoardMode {
                    ...
                    if let currentTab = viewModel.currentTab {
                        if currentTab.webView != nil {
                            // Active tab with web view - this is where downloads happen
                            WebViewContainer(
                                viewModel: viewModel,
        ...
        // Add the download animation overlay
        .downloadAnimation(
            isTriggered: $downloadAnimationManager.isTriggered,
            tappedPosition: $downloadAnimationManager.tappedPosition,
            targetPosition: CGPoint(
                x: 35,
                y: (NSScreen.main?.visibleFrame.size.height ?? 0) - 100
            ),
            icon: {
                ZStack {
                    // Outer glow effect
